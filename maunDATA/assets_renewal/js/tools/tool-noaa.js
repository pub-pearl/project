...
var ClientToolJS = (function() {
    ...
    var contentCtgs = [
        {ctg:'category' , grpCode:'NOACTG_1'},
        {ctg:'illegal'  ,empty:'불법유형', grpCode:'NOACTG_2'},
        {ctg:'cover'    ,empty:'덮개상태', grpCode:'NOACTG_3'},
        {ctg:'payload'  ,empty:'적재물', grpCode:'NOACTG_4'},
        {ctg:'curve'    ,empty:'회전쏠림', grpCode:'NOACTG_5'},
        {ctg:'course'    ,empty:'차량방향', grpCode:'NOACTG_6'}
    ];
    var metaCtgs = {
        'T' : {ctg:'time', empty:'조도'}
        ,'W' : {ctg:'weather', empty:'날씨'}
        ,'L' : {ctg:'lane', empty:'차선'}
    };
    var category = [];
    var metaCategory = {};
    ...
    function appendClassification($workingArea) {
        var categories = [
            {idx : 0, category : metaCategory.W, uuid : $.genUuid() }
            ,{idx : 1, category : metaCategory.T, uuid : $.genUuid() }
            ,{idx : 2, category : metaCategory.L, uuid : $.genUuid() }
        ];
            ...
            if(typeof contentList === 'object') {
                ...
                contentList.forEach(item => {
                    if(!$.isEmptyObject(item)) {
                        ...
                        else if(item.contentCtg == 'lane') {
                            item.category = metaCategory.L;
                            var objectHtmlTemp = $.templates("#metaDataTemplate").render(item);
                            var $workData = $workingArea.append(objectHtmlTemp);
                            if(!$.isEmptyObject(item.content)) {
                                $workData.find("select[name="+item.contentCtg+"]").last().val(item.content);
    ...
    function saveToDrawData(workData) {
        var rtnData = []; // FIXME 확인용 코드임. 삭제 요망.
                    ...
                    if(!$.isEmptyObject(item) && item.contentCtg == 'noa') { // object 세팅
                        if(!$.isEmptyObject(item.content)) {
                            ...
                            item.content.forEach(content => {
                                if(content.contentCtg === 'box') {
                                ...
                                } else if(content.contentCtg === 'category') { // 차량 종류
                                    ...
                                    // workingDataSet.category_name = content.content;
                                }
                            });
    ...
    function getData($this, preData) {
        ...
        if(category == 'noa') {
            ...
            contentCtgs.forEach(val => {
                ctgValue = $this.find("select[name="+val.ctg+"]").val();
